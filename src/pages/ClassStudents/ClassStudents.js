  ...
  form(({ students }) => ({
    ...
    component({
      render ({ props }) {
        ...
        const studentList = mapValues(
          (s, id) => ({ ...s, id }),
          students.value.students
        )
        const selected = (fields.selected.value || []).filter(
          id => students.value.students[id]
        )
...
const StudentMenu = component({
      ...
      <Flex align='space-between center' mb>
        ...
        <Tooltip message={!count && 'Select Students to Enable'}>
          <Button
            ...
            color='white'
            {...btnProps}
            onClick={actions.removeModal(users)}>
        ...
        {showPasswords ? (
          <Tooltip message={!count && 'Select Students to Enable'}>
            <Button
              disabled={!count}
              onClick={actions.printLoginModal(users)}
              bgColor='grey'>
  ...
  controller: {
    ...
    * removeModal ({ props, context }, users) {
      yield context.openModal(() => (
        <RemoveFromClassModal user={users} groupId={props.groupId} />
      ))
    },
    ...
    * passwordModal ({ props, context }, users) {
      yield context.openModal(() => (
        <PasswordModal user={users} group={props.group} />
      ))
    }
  }
})
